...
var config ={
    ...
    snrStart:1, // 1 snr  best quality
    snrEnd :15, // 15 snr worse quality
    finishIfReversalIs:7, // use 7 as recommended by Levit t , H. (1992).
    exportFileName:"dark_comfortable.csv",
    testStartAtSNR:7, // The first question will have snr of testStartAtSNR, to start at beginning put 1
    debug:true
}
...
function convertLogsToCSV() {
        ...
        result = '';
		//adding the comfortable brightness
		result+=`comfortable brightness: ${document.getElementById("Brightness_comfortable").value}\n`;
...
function addJNDQuestion(n,snrLevel){
	...
	var tempelate=`<fieldset id="fieldset_{0}">
						<label>{0}.&nbsp;Look at the following image and enter the three numbers that you see.</label>
						<div class="" style="margin-top:10px;">
							<div align="center">
								<img  src="{1}" width="250" height="250">
							</div>
							<div align="center"> <br/>
								<input type="number" id="num_{0}" name="num_{0}" required="" class="form-control myinput nospace" autocomplete="off">
							</div> <br/>
							<div class="main">
								<button type="button" class="btn btn-primary buttonCentered" id="bt{0}" onclick="submitAnsJnd({0},{2},\'{3}\');" >Next</button>
							</div>
						</div>
					</fieldset>`;
...
function submitAnsJnd(qNum,snr, correct_num){
	...
	if (!document.querySelector('input[name="num_'+qNum+'"]').value){
		alert("Please enter the number you see.");
		return;
	}
	//disable next button
...
function finished(){
	...
	template=`
		...
		&nbsp;&nbsp;&nbsp;
	 	<button class="btn btn-success btn-lg active buttonCenteredResults" style="display:none;" id="nextTask" onclick="window.location.href='../light_50/index.html'">Next Task</button>`
